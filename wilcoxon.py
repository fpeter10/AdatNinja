    ...
    while True:
            try: 
                ...
                myMethods.check_everything_string(table= table, table_path= table_path, value_to_check= group_col, 
                                                  parameter= "group_col", sep= sep, is_none_possible= False)
                ...
                elif 'as --group_col column not found!' in str(e):
...
# MAIN
def parse_stat():
    parser = argparse.ArgumentParser(description="Calculate the non parametric wilcoxon test between 2 groups in a name!",
    ...
    parser.add_argument("--output", required=False, help= myMethods.help_output(default_output= "wilcoxon_output"))
...
if __name__ == "__main__":
    ...
    if not args.output:
        ...
        args.output = f"{table_base}_wilcoxon_output"
    ...
    while True:
        try:
            workdir_save, table_save, output_file_save, name_col_save, value_col_save, group_col_save = calculate_wilcoxon_test(workdir, table, output, name_col, value_col, group_col, test_mode)
            break
    ...
    # mentés json fileba
    cmd = f"python wilcoxon.py --workdir {args.workdir} --table {args.table} --output {args.output} --name_col {args.name_col} --value_col {args.value_col} --group_col {args.group_col}"
