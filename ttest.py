...
# MAIN
def parse_stat():
    parser = argparse.ArgumentParser(description="Calculate the ttest between 2 groups for any names.",
    ...
    parser.add_argument("--output", help=myMethods.help_output(default_output= "ttest_output"))
...
if __name__ == "__main__":
    ...
    if not args.output:
        ...
        args.output = f"{table_base}_ttest_output"
    ...
    while True:
        try:
            workdir_save, table_save, output_file_save, name_col_save, value_col_save, group_col_save = calculate_t_test(workdir, table, output, name_col, value_col, group_col, test_mode)
            break
    ...
    # mentés json fileba
    cmd = f"python ttest.py --workdir {args.workdir} --table {args.table} --output {args.output} --name_col {args.name_col} --value_col {args.value_col} --group_col {args.group_col}"
