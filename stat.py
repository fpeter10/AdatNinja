...
# MAIN
def parse_stat():
    parser = argparse.ArgumentParser(description="Calculate the basic stats (mean, stdev, min, max, sum), for any names in any groups!",
    ...
    parser.add_argument("--output", required=False, help= myMethods.help_output(default_output= "_stat_output"))
    parser.add_argument("--name_col", required=False, help= myMethods.help_name(default_name= "id"))
    parser.add_argument("--value_col", required=False, help= myMethods.help_value(default_value= "value"))
    parser.add_argument("--group_col", required=False, help= myMethods.help_group(default_group= "group"))
...
if __name__ == "__main__":
    ...
    if not args.output:
        ...
        args.output = f"{table_base}_stat_output"
    ...
    while True:
        try:
            workdir_save, table_save, output_file_save, name_col_save, value_col_save, group_col_save = calculate_statistics(workdir, table, output, name_col, value_col, group_col, test_mode)
            break
    ...
    # mentés json fileba
    cmd = f"python stat.py --workdir {args.workdir} --table {args.table} --output {args.output} --name_col {args.name_col} --value_col {args.value_col} --group_col {args.group_col}"
